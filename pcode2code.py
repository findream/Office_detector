...
pcode2code is a module aiming at decompiling VBA pcode. It's based on Dr. Vesselin Vladimirov Bontchev(@VessOnSecurity)'s tool pcodedmp, which makes the disassembly, while this module is only an interface to print out readable VBA code.

How it works:
Basically, pcodedmp gives out a bytecode interpretation, representing operations made on a somewhat stack, with opcodes translation. This program simply parses the opcodes and manipulates the stack to rectonstruct the original VBA code.
...
class Operations:
    ...
    def linenum(self, *args):
        raise Pcode2codeException('not implemented linenum')
    ...
    def putrec(self):
        ...
        record_num = self.opstack.pop()
        chan = self.opstack.pop()
        self.opstack.push('Put ' + chan + ', ' + record_num + ', ' + record)
...
class Parser:
    ...
    def processInput_debugmode(self):
        """
...
def process(inputfile, outputfile=None, ispcodedump=False, linenum=False, isdebug=False):
    """
        The generic API function that every other module should use. 
        args: 
          - inputfile: the file to process, can be either an OLE file or a previous dump of pcodedmp module
          - outputfile: the place where to print the output. By default, the output is printed on stdout
          - ispcodedump: boolean indicating if the input file is a previous dump of pcodedmp module
          - linenum: boolean to indicate if we should add line numbers in the output. 
                     When True, line number is printed at the beginning. As such, the output is not a valid VBA code, 
                     and when pcode2code cannot interpret a line, then exception is printed without any precaution.
                     When False, the output should be a valid VBA code (minus limitations on disassembling). When the 
                     input cannot be processed for a said line, then the associated exception is printed in a comment.
          - isdebug: boolean used for debugging purpose. When True, exceptions are not handled, making it easier to find out bugs.
    """
    output_file = sys.stdout
    if outputfile is not None:     # check there to avoid unecessary processing
        try:
    ...
    if not ispcodedump: #in this case we are processing a pcodedmp dump
        try:
            from pcodedmp import pcodedmp
        except Exception as e:
            print('Error: Error trying to import pcodedmp module', file=sys.stderr)
            sys.exit(132)
        ...
        if PYTHON2:
            ...
            output = StringIO()
        #fake args class for pcodedmp    
        class pcodedmpargs:
            disasmOnly = True
            verbose = False
        try:
            pcodedmp.processFile(inputfile, pcodedmpargs, output_file=output)
        except Exception as e:
            print('Error: Error while running pcodedmp', file=sys.stderr)
            sys.exit(132)
        myinput = output.getvalue()

    inputParser = Parser(myinput)
    inputParser.parseInput()
    if isdebug:
        inputParser.processInput_debugmode()        
    else:           
        inputParser.processInput(linenum)

    toprint = inputParser.getOutput()
...
def main():
    """
        This function is only there to process arguments when the module is called directly. Then, the generic API is simply called with parsed args
    """
    try:
        import argparse
    except Exception as e:
    ...
    parser = argparse.ArgumentParser(description='Disassemble the p-code of VBA-containing documents.')
    parser.add_argument('-v', '--version', action='version', version='%(prog)s version {}'.format(__VERSION__))
    parser.add_argument('-p', '--pcodedump', action='store_true', default=False, help='when a pcodedmp previous dump is to be processed, use this option')
    parser.add_argument('-n', '--linenum', action='store_true', default=False, help='print line numbers in output')
    parser.add_argument('-o', '--output', dest='outputfile', default=None, help='Output file name')
    parser.add_argument('-d', '--debug', action='store_true', default=False, help='debug mode not hiding potential exceptions, used for dev')
    parser.add_argument('inputfile', help='File to process')
    args = parser.parse_args()
    process(args.inputfile, args.outputfile, args.pcodedump, args.linenum, args.debug)
